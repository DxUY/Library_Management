...
int tongquahan(NodeDG* p)//tinh tong ngay qua han
{
	NodeMuonTra* b = p->mt.pHead;
	int total = 0;
	for (int i = 0; i < p->mt.n; i++)
	{
		NgayThang tmp = b->data.ngaytra;
		if (tmp.ngay == 0 || tmp.thang == 0 || tmp.nam == 0)
		{
			b = b->pNext;
		}
		else
		{
			if (compareDate(b->data.ngaytra))
			{
				total += countOverdueDay(b->data.ngaytra);
			}
			b = b->pNext;
		}
	}
...
void DanhSachQuaHan(TREE_DG t)
{
	...
	while (p != NULL)
	{
		...
		ArrQuaHan[ndg].songayquahan = tongquahan(p);
		ndg++;
