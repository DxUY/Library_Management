...
void selectionSort_DS(LIST_DAUSACH& lDS) {
	...
	for (i = 0; i <= lDS.n - 1; i++) {
		...
		min_idx = i;
		for (j = i + 1; j <= lDS.n; j++) {
			// Compare genres first
			if (strcmp(lDS.ListDS[j]->info.theloai.c_str(), lDS.ListDS[min_idx]->info.theloai.c_str()) < 0) {
				min_idx = j;
			}
			else if (strcmp(lDS.ListDS[j]->info.theloai.c_str(), lDS.ListDS[min_idx]->info.theloai.c_str()) == 0) {
				// If genres are the same, compare titles
