...
void Introduce()
{
	...
	gotoxy(xTen, yTen);
	cout << "HOTEN : HOANG MINH DUY";
	gotoxy(xTen, yTen + 1);
	cout << "MSSV  : N22DCDT010";
	gotoxy(xTen, yTen + 2);
	cout << "LOP   : E22CQCN01-N";

	SetColor(LIGHT_GREEN);
	gotoxy(xTen + 30, yTen);
	cout << "HOTEN : LAI PHUC TUAN";
	gotoxy(xTen + 30, yTen + 1);
	cout << "MSSV  : N22DCDT072";
	gotoxy(xTen + 30, yTen + 2);
	cout << "LOP   : E22CQCN01-N";
...
label:
	...
	else if (check == 1)
	{
		...
		//	temp->mt.pHead->data.ngaytra = HandleLogicalTime(temp->mt.pHead->data.ngaymuon);
			//cout << temp->mt.pHead->data.ngaytra.nam; system("pause");
...
{
	system("color 0F");// nen den chu mau vang
	ShowCur(false);
	string temp;
